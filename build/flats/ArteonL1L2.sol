...
contract ArteonStorage {
  using Counters for Counters.Counter;

  Counters.Counter internal _tokenIds;

  address internal ARTEON;

  bytes32 public constant MINT_ROLE = keccak256("MINT_ROLE");
  uint256 internal _blockTime = 60;
  string internal _uri;
  uint256 public percentSettings = 1;
...
contract ArteonL1L2 is ArteonStorage, Context, ERC165, EIP712Base, IERC1155, IERC1155MetadataURI, AccessControl, Pausable {
  using Address for address;

  modifier isListed(uint256 id, uint256 tokenId) {
    require(lists[id][tokenId].listed != 0, 'NOT_LISTED');
    _;
  }

  constructor(address arteon, string memory uri_) {
    ARTEON = arteon;
  ...
  function _mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal virtual {
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      require(_totalSupply[ids[i]] < amounts[ids[i]]);
  ...
  function _mint(address to, uint256 id, uint256 amount, bytes memory data) internal virtual {
    require(to != address(0), "mint to the zero address");
    require(_totalSupply[id] < amount);
  ...
  function _removeBalance(address from, uint256 id, uint256 amount, uint256 burns) internal {
    ...
    require(balance >= amount, "insufficient balance for transfer/burn");
  ...
  function _beforeTokenTransfer(
    ...
    function buy(uint256 id, uint256 tokenId) external isListed(id, tokenId) {
      ...
      require(IERC20(ARTEON).balanceOf(account) >= price, 'ArteonExchange: NOT_ENOUGH_TOKENS');

      SafeERC20.safeTransferFrom(IERC20(ARTEON), account, owner, price);
