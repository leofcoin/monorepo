...
contract ArtOnlineLottery is Initializable, LotteryStorage {
  ...
  function setLotterySize(uint16 lotterySize_) external onlyManager {
    _lotterySize = lotterySize_;
  }

  function lotterySize() external view virtual returns (uint16) {
    return _lotterySize;
  }
  ...
  function createLottery(uint256 startTime_, uint256 endTime_, uint256 _costPerTicket, uint16[] calldata _prizeDistribution) external virtual onlyManager {
    ...
    for (uint16 i = 0; i < _prizeDistribution.length; i++) {
    ...
    uint16[] memory _totalMatches = new uint16[](_lotterySize);
  ...
  function revealWinningNumbers(uint256 lottery_, uint16[] memory winningNumbers_, uint16[] memory totalMatches_, string memory proof) external onlyManager {
    ...
    _lotteries[lottery_].winningNumbers = winningNumbers_;
    ...
    emit LotteryClose(lottery_, winningNumbers_, _burns[lottery_]);
  }

  function buyTickets(uint256 id_, uint256 tickets, uint16[] memory numbers_) external notContract() {
  ...
  function _getNumberOfMatching(uint16[] memory _usersNumbers, uint16[] memory _winningNumbers) internal pure returns(uint256 noOfMatching) {
     for (uint16 i = 0; i < _winningNumbers.length; i++) {
