...
;(async () => {
  ...
  const createMessage = async (src, params = []) => {
    ...
    return chain.createContractMessage(
      peernet.id,
      `return ${contract.toString().replace(/export{([A-Z])\w+ as default}/g, '')}`,
      params
    )
  }
  ...
  const nameService = await createMessage('./dist/contracts/nameService.js', [
    factory.hash,
    nativeToken.hash,
    validators.hash,
    BigNumber.from('1000')
