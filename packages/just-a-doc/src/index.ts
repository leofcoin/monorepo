...
const generate = async (file, output) => {
      ...
      if (line.includes('@') && !isExample) {
        ...
        if (line.includes('@return')) {
          ...
          token.return = {
            ...
            value,
            type,
            description: description.slice(description.indexOf(name) + name.length).trim()
          }
          value = ''
        }
  ...
  await writeFile(
    join(output, base.endsWith('.ts') ? `${base}.js` : base),
    `export default ${JSON.stringify(tokens, null, '\t')}`
  )
  // } catch(error) {
