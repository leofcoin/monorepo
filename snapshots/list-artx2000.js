...
(async () => {
  ...
  const ids = list.map(id => ethers.BigNumber.from('2'))
  ...
  for (const id of ids) {
    ...
    if (list[index].id < 207) {
      try {
        let tx = await exchangeContract.list(ethers.BigNumber.from('2'), ethers.BigNumber.from(list[index].id), ethers.utils.parseUnits('50500'))
        await tx.wait()
      } catch (e) {
